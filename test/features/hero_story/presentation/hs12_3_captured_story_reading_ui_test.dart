...
void main() {
  ...
  setUp(() async {
    ...
    tempDir = Directory.systemTemp.createTempSync('hs12-3-ui-');
  ...
  Future<void> captureThroughHeroStory(
    ...
    await tester.runAsync(() async {
      ...
      controller.setGrantAiTransformation(grantAi);
  ...
  testWidgets('opening Hero Story does not auto-transcribe', (tester) async {
    ...
    expect(find.byKey(const ValueKey('hero-story-reading-section')), findsNothing);
    final story = (await stories.findAll()).single;
    expect(jobStore.findByStory(story.id), isEmpty);
    expect(await readings.findByStoryId(story.id), isNull);
  });

  testWidgets('Understand my story shows grounded reading', (tester) async {
    final container = buildContainer();
    addTearDown(container.dispose);
    ...
    expect(find.byKey(const ValueKey('hero-story-reading-challenge')), findsOneWidget);
    expect(
      find.byKey(const ValueKey('hero-story-reading-turning-point')),
      findsOneWidget,
    );
    expect(find.byKey(const ValueKey('hero-story-reading-outcome')), findsOneWidget);
  ...
  testWidgets('AI failure keeps Play my recording and allows retry', (
    tester,
  ) async {
    readingAdapter = InMemoryCapturedStoryReadingAdapter(
      forcedFailureMessage: 'reading failed for test',
    );
    final container = buildContainer();
    addTearDown(container.dispose);
    ...
    await tester.runAsync(() async {
      for (var i = 0; i < 40; i++) {
        ...
        final phase =
            container.read(heroStoryUnderstandingProvider(storyId)).phase;
        if (phase == HeroStoryUnderstandingPhase.failed) {
          break;
        }
        await Future<void>.delayed(const Duration(milliseconds: 25));
      }
    ...
    expect(
      find.byKey(const ValueKey('hero-story-understanding-error')),
      findsOneWidget,
    );
    expect(find.text('Play my recording'), findsOneWidget);
    expect(find.text('Retry understanding'), findsOneWidget);
    expect(find.byKey(const ValueKey('hero-story-reading-section')), findsNothing);
    ...
    // Recover adapter via provider override on a fresh container is hard;
    // verify Back still works after failure.
  ...
  testWidgets('retry after failure can succeed', (tester) async {
    final toggle = _ToggleReadingPort();
    readingAdapter = toggle;
    final container = buildContainer();
    addTearDown(container.dispose);
    ...
    await tester.runAsync(() async {
      for (var i = 0; i < 40; i++) {
        final phase =
            container.read(heroStoryUnderstandingProvider(storyId)).phase;
        if (phase == HeroStoryUnderstandingPhase.failed) break;
        await Future<void>.delayed(const Duration(milliseconds: 25));
      }
    });
    await tester.pumpAndSettle();
    ...
    await tester.runAsync(() async {
      for (var i = 0; i < 40; i++) {
        final phase =
            container.read(heroStoryUnderstandingProvider(storyId)).phase;
        if (phase == HeroStoryUnderstandingPhase.ready) break;
        await Future<void>.delayed(const Duration(milliseconds: 25));
      }
    });
    await tester.pumpAndSettle();
...
final class _ToggleReadingPort implements CapturedStoryReadingPort {
  bool fail = true;
  ...
  Future<CapturedStoryReadingDraft> generate(
    ...
    if (fail) {
      throw const CapturedStoryReadingException('temporary');
    }
