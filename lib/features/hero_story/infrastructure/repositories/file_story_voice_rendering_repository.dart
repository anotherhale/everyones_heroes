...
/// File-backed [StoryVoiceRenderingRepository] (HS.12.6).
///
/// Files under `{root}/story_voice_renderings/{id}.json`.
/// Latest-per-story index under `{root}/story_voice_renderings/_by_story.json`.
final class FileStoryVoiceRenderingRepository
    implements StoryVoiceRenderingRepository {
  FileStoryVoiceRenderingRepository({required Directory rootDirectory})
      : _renderingsDirectory = Directory(
          p.join(rootDirectory.path, 'story_voice_renderings'),
        );

  final Directory _renderingsDirectory;
  final Map<StoryVoiceRenderingId, StoryVoiceRendering> _cache = {};
    ...
    await for (final entity in _renderingsDirectory.list()) {
      ...
      try {
        ...
        throw FormatException(
          'Corrupt StoryVoiceRendering file ${entity.path}: $e',
        );
      }

      if (decoded is! Map) {
        throw FormatException(
          'Corrupt StoryVoiceRendering file ${entity.path}: '
          'expected JSON object.',
        );
      }

      final rendering = StoryVoiceRenderingSnapshotMapper.fromJson(
      ...
      if (decoded is Map) {
        for (final entry in decoded.entries) {
          _latestByStory[StoryId(entry.key as String)] =
              StoryVoiceRenderingId(entry.value as String);
        }
      }
    } else {
  ...
  File _fileFor(StoryVoiceRenderingId id) =>
  ...
  Future<void> save(StoryVoiceRendering rendering) async {
    ...
    await file.writeAsString(
      jsonEncode(StoryVoiceRenderingSnapshotMapper.toJson(rendering)),
      flush: true,
    );
    _cache[rendering.id] = rendering;
    _latestByStory[rendering.storyId] = rendering.id;
